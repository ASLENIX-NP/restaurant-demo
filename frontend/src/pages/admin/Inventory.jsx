  ...
  const handleFormSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      const newItem = {
        code: newCode,
        image:
          "https://images.unsplash.com/photo-1542838132-92c53300491e?q=80&w=120&auto=format&fit=crop", // default mockup image
        name: formName,
        category: formCategory,
        unit: formUnit,
        qty: parseFloat(formQty).toFixed(2),
                  ...
                  {items.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-center">
                        <span
                          className={`px-2.5 py-1 rounded-md text-[11px] font-black uppercase tracking-wider border ${getStatusBadge(
                            item.status
                          )}`}
                        >
                          {item.status}
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={handleOpenAddModal}
                  className="flex flex-col items-center justify-center gap-2 p-4 bg-slate-50 hover:bg-purple-50 hover:text-purple-600 text-slate-600 rounded-xl border border-slate-100 transition-colors font-semibold text-xs"
                >
                  <Plus size={20} /> Add Item
                </button>
                <button
                  onClick={() => handlePlaceholderAction("Stock Adjustment")}
                  className="flex flex-col items-center justify-center gap-2 p-4 bg-slate-50 hover:bg-orange-50 hover:text-orange-600 text-slate-600 rounded-xl border border-slate-100 transition-colors font-semibold text-xs text-center leading-tight"
                >
                  <Settings2 size={20} /> Stock Adjust
                </button>
                <button
                  onClick={() => handlePlaceholderAction("Add Category")}
                  className="flex flex-col items-center justify-center gap-2 p-4 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 text-slate-600 rounded-xl border border-slate-100 transition-colors font-semibold text-xs text-center leading-tight"
                >
                  <FolderPlus size={20} /> Category
                </button>
                <button
                  onClick={() => handlePlaceholderAction("Suppliers")}
                  className="flex flex-col items-center justify-center gap-2 p-4 bg-slate-50 hover:bg-blue-50 hover:text-blue-600 text-slate-600 rounded-xl border border-slate-100 transition-colors font-semibold text-xs"
                >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-slide-in">
            ...
            <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50/50">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-600 bg-white p-1 rounded-lg border border-slate-200 shadow-sm"
              >
                <X size={16} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] flex justify-center items-center p-4 transition-opacity">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center animate-slide-in">
            ...
            <p className="text-sm text-slate-500 font-medium mb-6">
              ...
              <strong>{itemToDelete}</strong>? This action cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => {
