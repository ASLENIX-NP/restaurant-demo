...
const Inventory = () => {
 ...
 try {
 const { data } = await apiClient.get("/api/inventory/logs");
 ...
 try {
 await apiClient.put(`/api/inventory/${adjustmentItemCode}/adjust`, {
 type: adjustmentType,
 qty: adjustmentQty,
 });
 ...
 const headers = ["Item Code","Item Name","Category","Unit","Stock Qty","Status","Last Updated"];
 ...
 <button onClick={handleExportCSV} className="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-semibold text-sm px-4 py-2.5 rounded-xl shadow-sm flex items-center gap-2 transition-all">
 ...
 <img loading="lazy" src={item.image} alt={item.name} className="w-10 h-10 rounded-lg object-cover border border-slate-200 shadow-sm" />
