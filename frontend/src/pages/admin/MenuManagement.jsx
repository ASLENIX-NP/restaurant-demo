...
const Menu = () => {
  ...
  const activeItems = menuItems.filter((item) => item.status === true).length;
  const inactiveItems = menuItems.filter((item) => item.status === false).length;
  const avgPrice = totalItems > 0
      ? Math.round(menuItems.reduce((sum, item) => sum + Number(item.price || 0), 0) / totalItems)
      : 0;
    ...
    <div className="min-h-screen bg-slate-50 p-8 text-slate-800 font-sans">
      <main className="max-w-[1600px] mx-auto pb-12">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-[28px] font-bold text-slate-900 tracking-tight">Menu Management</h1>
            <p className="text-slate-400 text-sm mt-0.5 font-medium">
              Dashboard <span className="mx-1.5 text-slate-300">&gt;</span> Menu Management
            </p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col gap-3">
            ...
            <div>
              <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Total Items</h4>
              <h2 className="text-xl font-black text-slate-900">{totalItems}</h2>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col gap-3">
            ...
            <div>
              <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Categories</h4>
              <h2 className="text-xl font-black text-slate-900">{totalCategories}</h2>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col gap-3">
            ...
            <div>
              <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Available</h4>
              <h2 className="text-xl font-black text-slate-900">{activeItems}</h2>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col gap-3">
            ...
            <div>
              <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Unavailable</h4>
              <h2 className="text-xl font-black text-slate-900">{inactiveItems}</h2>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col gap-3">
            ...
            <div>
              <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Avg Price</h4>
              <h2 className="text-xl font-black text-slate-900">Rs. {avgPrice}</h2>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col gap-3">
            ...
            <div>
              <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Popular Dish</h4>
              <h2 className="text-sm font-black text-slate-900 truncate mt-1">{popularDish}</h2>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between bg-slate-50/50">
            <div className="flex bg-white border border-slate-200 p-1 rounded-xl shadow-sm">
              <button className="px-4 py-1.5 rounded-lg text-sm font-bold bg-slate-900 text-white shadow-sm">All Items</button>
          ...
          <div className="overflow-x-auto min-h-[400px]">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                ...
                <p className="font-semibold text-sm">Loading Menu Products...</p>
              </div>
            ) : (
                  ...
                  {filteredItems.length === 0 ? (
                    ...
                    filteredItems.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="p-4 pl-6 font-bold text-slate-400 text-xs">{item.id}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4 transition-opacity">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-slide-in">
            ...
            <form onSubmit={handleAddItem} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Item Name *</label>
                <input 
                  type="text" 
                  required 
                  value={newItem.name} 
