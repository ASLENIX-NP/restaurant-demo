...
const Reservations = () => {
  const { tables, editTable, updateTableStatus } = useTables() || { tables: [] };
  ...
  useEffect(() => {
    localStorage.setItem("restaurant_reservations", JSON.stringify(reservationsData));
  ...
  const handleAddReservation = (e) => {
    ...
    if (res.table && tables && (editTable || updateTableStatus)) {
      const tableObj = tables.find(
        (t) => (t.name || `Table ${t.id}`).toLowerCase() === res.table.toLowerCase()
      );
  ...
  const handleStatusChange = (id, newStatus) => {
    setReservationsData(reservationsData.map(r => r.id === id ? { ...r, status: newStatus } : r));
    setSelectedReservation(prev => prev && prev.id === id ? { ...prev, status: newStatus } : prev);
  }
    ...
    <div className="min-h-screen bg-slate-50 p-8 text-slate-800 font-sans">
      <main className="max-w-[1600px] mx-auto pb-12">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-[28px] font-bold text-slate-900 tracking-tight">Manage Reservations</h1>
            <p className="text-slate-400 text-sm mt-0.5 font-medium">
              Cashier Panel <span className="mx-1.5 text-slate-300">&gt;</span> Reservations
            </p>
          </div>
          <button 
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all border ${selectedReservation.status === "Confirmed" ? "bg-emerald-500 text-white border-emerald-600 shadow-md" : "bg-white text-slate-600 border-slate-200 hover:bg-emerald-50"}`}
                  >
                    Confirm
                  </button>
                  <button 
                    onClick={() => handleStatusChange(selectedReservation.id, "Pending")}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all border ${selectedReservation.status === "Pending" ? "bg-amber-500 text-white border-amber-600 shadow-md" : "bg-white text-slate-600 border-slate-200 hover:bg-amber-50"}`}
                  >
                    Pending
                  </button>
                  <button 
                    onClick={() => handleStatusChange(selectedReservation.id, "VIP")}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all border ${selectedReservation.status === "VIP" ? "bg-purple-500 text-white border-purple-600 shadow-md" : "bg-white text-slate-600 border-slate-200 hover:bg-purple-50"}`}
                  >
                    VIP
                  </button>
                </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4 transition-opacity" onClick={() => setShowAddModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-slide-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50/50">
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <CalendarPlus size={18} className="text-purple-500" /> New Booking
              </h2>
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-slate-600 bg-white p-1 rounded-lg border border-slate-200 shadow-sm transition">
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleAddReservation} className="p-6 space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Guest Name *</label>
                <input type="text" required value={newRes.name} onChange={(e) => setNewRes({...newRes, name: e.target.value})} className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl px-4 py-2.5 outline-none focus:border-purple-500 transition-all font-medium text-sm" placeholder="e.g. Jane Doe" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Table</label>
                  <select value={newRes.table} onChange={(e) => setNewRes({...newRes, table: e.target.value})} className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl px-4 py-2.5 outline-none focus:border-purple-500 transition-all font-medium text-sm cursor-pointer">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Guests *</label>
                  <input type="number" required min="1" value={newRes.guests} onChange={(e) => setNewRes({...newRes, guests: e.target.value})} className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl px-4 py-2.5 outline-none focus:border-purple-500 transition-all font-medium text-sm" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={newRes.time} onChange={(e) => setNewRes({...newRes, time: e.target.value})} className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl px-4 py-2.5 outline-none focus:border-purple-500 transition-all font-medium text-sm" placeholder="7:00 PM" />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    value={newRes.phone} 
                    onChange={(e) => setNewRes({...newRes, phone: e.target.value.replace(/\D/g, '').slice(0, 10)})} 
              ...
              <div>
                ...
                <select value={newRes.status} onChange={(e) => setNewRes({...newRes, status: e.target.value})} className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl px-4 py-2.5 outline-none focus:border-purple-500 transition-all font-medium text-sm cursor-pointer">
              ...
              <div>
                ...
                <textarea rows="2" value={newRes.notes} onChange={(e) => setNewRes({...newRes, notes: e.target.value})} className="w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl px-4 py-3 outline-none focus:border-purple-500 transition-all font-medium text-sm resize-none" placeholder="Special requests..."></textarea>
              </div>

              <div className="flex gap-3 pt-4 mt-2 border-t border-slate-100">
                <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 bg-white border border-slate-200 text-slate-700 font-bold py-3 rounded-xl hover:bg-slate-50 transition">
                  Cancel
                </button>
                <button type="submit" className="flex-1 bg-slate-900 text-white font-bold py-3 rounded-xl hover:bg-slate-800 shadow-md transition">
                  Create Booking
                </button>
              </div>
            </form>
