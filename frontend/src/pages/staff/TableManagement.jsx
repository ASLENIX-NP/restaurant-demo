...
const Tables = () => {
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case "Available":
        return { bg: "bg-emerald-50", text: "text-emerald-600", border: "border-t-emerald-400" };
      case "Occupied":
        return { bg: "bg-rose-50", text: "text-rose-600", border: "border-t-rose-400" };
      case "Reserved":
        return { bg: "bg-amber-50", text: "text-amber-600", border: "border-t-amber-400" };
      case "Cleaning":
        return { bg: "bg-blue-50", text: "text-blue-600", border: "border-t-blue-400" };
      default:
        return { bg: "bg-slate-50", text: "text-slate-600", border: "border-t-slate-400" };
    }
  };
    ...
    <div className="tables-page p-4 sm:p-6 md:p-8 bg-slate-50 min-h-screen font-sans text-slate-800">
      ...
      <div className="tables-header flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5 font-medium">Manage layout and live capacity</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 mb-6 md:mb-8">
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center gap-5 hover:shadow-md hover:-translate-y-1 transition-all">
          ...
          <div>
            <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1">Total Tables</h4>
            <h2 className="text-3xl font-black text-slate-900 leading-none">{totalTables}</h2>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center gap-5 hover:shadow-md hover:-translate-y-1 transition-all">
          ...
          <div>
            <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1">Available</h4>
            <h2 className="text-3xl font-black text-slate-900 leading-none">{availableCount}</h2>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center gap-5 hover:shadow-md hover:-translate-y-1 transition-all">
          ...
          <div>
            <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1">Occupied</h4>
            <h2 className="text-3xl font-black text-slate-900 leading-none">{occupiedCount}</h2>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center gap-5 hover:shadow-md hover:-translate-y-1 transition-all">
          ...
          <div>
            <h4 className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1">Reserved</h4>
            <h2 className="text-3xl font-black text-slate-900 leading-none">{reservedCount}</h2>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="px-5 pb-5 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="truncate">
                  ...
                  <p className="text-sm font-bold text-slate-900 truncate">{table.customer}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 border-t border-slate-100 divide-x divide-slate-100 bg-slate-50/50">
              <button
                ...
                onClick={(e) => { e.stopPropagation(); handleOpenView(table); }}
              >
                <Eye size={14} /> View
              </button>
              <button
      ...
      {activeModal && (
        <div className="modal-overlay fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4 transition-opacity" onClick={closeModal}>
          <div className="modal-content bg-white rounded-2xl shadow-xl w-full max-w-sm sm:max-w-md overflow-hidden flex flex-col max-h-[90vh] animate-slide-in" onClick={(e) => e.stopPropagation()}>
            {/* VIEW DETAILS MODAL */}
            {activeModal === "view" &&
              selectedTable &&
              (() => {
                ...
                return (
                  <>
                    <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50/50">
                      ...
                      <button onClick={closeModal} className="text-slate-400 hover:text-slate-600 bg-white p-1.5 rounded-lg border border-slate-200 shadow-sm transition">
                        <X size={16} />
                      </button>
                    </div>
                    ...
                    <div className="p-6 overflow-y-auto space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                          ...
                          <span className="font-bold text-slate-900 flex items-center gap-1.5"><Utensils size={14} className="text-slate-400"/> {currentViewingTable.seats} Seats</span>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                          <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Customer</span>
                          <span className="font-bold text-slate-900 flex items-center gap-1.5 truncate"><User size={14} className="text-slate-400"/> {currentViewingTable.customer}</span>
                        </div>
                      </div>
            ...
            {(activeModal === "add" || activeModal === "edit") && (
              ...
              >
                  ...
                  {activeModal === "add"
                    ...
                    <button type="button" onClick={closeModal} className="text-slate-400 hover:text-slate-600 bg-white p-1.5 rounded-lg border border-slate-200 shadow-sm transition">
                      <X size={16} />
                    </button>
                  </div>
